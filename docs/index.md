...
The pair of analog pins, A4 and A5, on the Nano are usually used for I2C communications according to the board pinout schematics. Hence, pins A4 and A5 are connected to SDA and SCL on the MPU6050 board respectively. The external interrupt pin of the MPU6050 is connected to the D2 digital Nano pin.
...
The image below, courtesy again of [Martin White](https://mjwhite8119.github.io/Robots/mpu6050), shows the format of the DMP FIFO buffer. 
...
The steps for calculating the MPU6050 board offsets were mainly obtained from the forum post [here](https://www.i2cdevlib.com/forums/topic/91-how-to-decide-gyro-and-accelerometer-offsett/) and modified for this project. The two main steps are as follows, 
...
This is the expected position of the gimbal when it's stationary. The axis of rotation of the servo motors are also labelled in the image above.
...
- Using a smaller buck or voltage regulator. The buck regulator used here barely fit into the housing for the electronic components.
- Using a smaller battery, like a Lithium Ion Polymer (Lipo) battery, to reduce the space occupied by the battery in the housing. 

Another suggestion would be to explore connecting the external battery power (within a range of [7-12V](https://docs.arduino.cc/hardware/nano)) directly to the Vin pin of the Arduino Nano, and relying on the internal 5V regulator instead. If feasible, this eliminates the need for an external buck regulator thereby making the housing more spacious.
